...
def marginalization_report(model, motifs, X, output_dir, minimal=False):
	...
	for i, (name, pwm) in enumerate(motifs):
		...
		Xc_attr_before, Xc_attr_after = marginalize(CountWrapper(model).float(), 
			X.float(), motif, func=deep_lift_shap, n_shuffles=1)
	...
	ap_before, ap_after, ap_diff = multistack([ap_before, ap_after, ap_diff])
	ac_before, ac_after, ac_diff = multistack([ac_before, ac_after, ac_diff])
	...
	apb_lim, apa_lim, apd_lim = multilims([ap_before, ap_after, ap_diff])
	acb_lim, aca_lim, acd_lim = multilims([ac_before, ac_after, ac_diff])
	...
	for i, idx in enumerate(idxs):
		...
		if not minimal:
			_plot_attributions(ap_before[idx], apb_lim, oname + pab + ".png")
			_plot_attributions(ap_after[idx], apa_lim, oname + paa + ".png")

			_plot_attributions(ac_before[idx], acb_lim, oname + cab + ".png")
			_plot_attributions(ac_after[idx], aca_lim, oname + caa + ".png")

		_plot_attributions(ap_diff[idx], apd_lim, oname + pad + ".png")
		_plot_attributions(ac_diff[idx], acd_lim, oname + cad + ".png")
