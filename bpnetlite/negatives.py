...
def calculate_perc(sequence, width, chars):
	"""Get the percentage of characters in `sequence` being in `chars`.
...
def extract_and_filter_chrom(fasta, chrom, in_window, out_window, 
	...
	gc_perc = calculate_perc(sequence, in_window, ('G', 'C'))
	n_perc = calculate_perc(sequence, in_window, ('N',))
	...
	for idx in idxs:
		gc_percs[gc_perc[idx]].append(idx)
...
def extract_matching_loci(loci, fasta, in_window=2114, out_window=1000, 
	...
	f = delayed(extract_and_filter_chrom)
